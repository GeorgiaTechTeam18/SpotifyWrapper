...
def setEmailAndUsernameIfProvided(tokens, email, username):
    if email is not None and username is not None:
        ...
        tokens.save(
            update_fields=[
                "spotify_account_username",
                "spotify_account_email"])
...
def update_or_create_user_tokens(
    ...
    expires_in,
    refresh_token,
    spotify_account_email=None,
    spotify_account_username=None,
):
    tokens = get_user_tokens(user)
    expires_in = timezone.now() + datetime.timedelta(seconds=expires_in)

    # creates a new token if the email doesn't equal the current token's
    # account email
    if tokens and (
        spotify_account_email is None
        or tokens.spotify_account_username == spotify_account_username
    ):
        ...
        setEmailAndUsernameIfProvided(
            tokens, spotify_account_email, spotify_account_username
        )
