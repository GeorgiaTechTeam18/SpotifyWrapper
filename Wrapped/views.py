...
def delete_wrap(request, wrap_id):
    ...
    if request.user == wrap.user:
        ...
        return JsonResponse(
            {"message": "You do not have permission to delete this wrap"}, status=403
        )
...
def create_wrap(request, time_range="None"):
    ...
    if artist_response.status_code == 200:
        ...
        return HttpResponseServerError(
            request,
            "Wrapped/get_top.html",
            {
                "error": "Failed to retrieve top artists. Try re-linking your spotify in the profile page"
            },
        )
    ...
    wrap, create = SpotifyWrap.objects.get_or_create(
        user=request.user, title="Spotify Wrapped"
    )
