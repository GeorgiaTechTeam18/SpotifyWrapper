...
def select_tracks(tracks, artists, genres):
    ...
    second = random.choice([track for track in tracks[1:6] if track['is_playable']])
    playable.remove(second)

    third_candidates = [track for track in playable if track['artist_name'] in [artist['name'] for artist in artists]]
    third = random.choice(third_candidates) if third_candidates else random.choice(playable)
    playable.remove(third)

    top_genre = genres[0][0]
    fourth_candidates = [track for track in playable if any(top_genre in genre for genre in next((artist['genres'] for artist in artists if artist['name'] == track['artist_name']), []))]
    fourth = random.choice(fourth_candidates) if fourth_candidates else random.choice(playable)
